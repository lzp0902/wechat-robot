package com.lzp.wechat.service;
...
import com.lzp.wechat.exception.WechatException;
import com.lzp.wechat.model.WechatContact;
import com.lzp.wechat.model.WechatMeta;
import com.lzp.wechat.robot.Robot;
import com.lzp.wechat.robot.TuLingRobot;
import com.lzp.wechat.util.Matchers;
...
public class WechatServiceImpl implements WechatService {
	...
	public WechatContact getContact(WechatMeta wechatMeta) {
		...
		HttpRequest request = HttpRequest.post(url).contentType("application/json;charset=utf-8")
				.header("Cookie", wechatMeta.getCookie()).readTimeout(10000).connectTimeout(10000).send(body.toString());
	...
	public void handleMsg(WechatMeta wechatMeta, JSONObject data) {
		...
		for (int i = 0, len = AddMsgList.size(); i < len; i++) {
			...
			Map<String,String> UserMap = getUserInfo(msg.getString("FromUserName"));
			String name = UserMap.get("RemarkName");
			String WeChatId = UserMap.get("WeChatId");
			String content = msg.getString("Content");
			List<String> WeChatIdList = Constant.FILTER_USERS;
//			List<String> WeChatIdList = new ArrayList<String>();
			for(String user:WeChatIdList) {
				if (user.equals(WeChatId) || user.equals(name)) {
...
//						content = peopleContent[1];
			...
			if (isloadRobot) {
				if (msgType == 51) {
					...
					HttpRequest.get(imgUrl).header("Cookie", wechatMeta.getCookie()).readTimeout(10000).connectTimeout(10000).receive(new File(imgDir + "/" + msgId+".jpg"));
					webwxsendmsg(wechatMeta, "傻强还不支持图片呢", msg.getString("FromUserName"));
				} else if (msgType == 34) {
					webwxsendmsg(wechatMeta, "傻强还不支持语音呢", msg.getString("FromUserName"));
				} else if (msgType == 42) {
				...
				isloadRobot = true;
//				LOGGER.info("拦截信息：" + "FromUserName：" + (String)msg.getString("FromUserName") + "\nWeChatId：" + WeChatId  + "\n" + name + ": " + content + "\n文本类型：" + msgType);
			}
		}
	}
	...
	private void webwxsendmsg(WechatMeta meta, String content, String to) {
		...
		HttpRequest request = HttpRequest.post(url).contentType("application/json;charset=utf-8")
				.header("Cookie", meta.getCookie()).readTimeout(10000).connectTimeout(10000).send(body.toString());
	...
	private Map<String,String> getUserInfo(String id) {
		...
		for (int i = 0, len = Constant.CONTACT.getMemberList().size(); i < len; i++) {
			...
			if (member.getString("UserName").equals(id)) {
				...
				if (StringKit.isNotBlank(member.getString("Alias"))) {
					WeChatId = member.getString("Alias");
				}
				userMap.put("RemarkName", name);
				userMap.put("WeChatId", WeChatId);
				return userMap;
			}
		}
		return userMap;
